...
### --- UI do Módulo de Filtros (VERSÃO FINAL COM LOTE E LOGOS) --- ###
filtrosUI <- function(id, opcoes_distrito, opcoes_lote, opcoes_nome, opcoes_modalidade, opcoes_concedente) {
  ns <- NS(id)
  ...
  tagList(
    ...
    div(style = "display: flex; flex-direction: column; height: 100%;", 
        ...
        div(class = "sidebar-logos",
            tags$img(src = "imagens/logo_pref.png", class = "sidebar-logo"),
            tags$img(src = "imagens/logo_spp.png", class = "sidebar-logo")
        )
    )
  )
}
...
### --- Server do Módulo de Filtros (VERSÃO FINAL COM LOTE) --- ###
filtrosServer <- function(id, dados_brutos) {
  moduleServer(id, function(input, output, session) {
    
    # --- LÓGICA DOS FILTROS EM CASCATA ---
    
    # 1. OBSERVADOR PARA ATUALIZAR AS OPÇÕES DE DISTRITO
    observeEvent(c(input$filtro_modalidade, input$filtro_concedente, input$filtro_nome, input$filtro_lote), {
      dados_intermediarios <- dados_brutos
      if (input$filtro_modalidade != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(ppp_modali == input$filtro_modalidade)
      }
      if (input$filtro_concedente != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(ppp_conced == input$filtro_concedente)
      }
      if (input$filtro_nome != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(ppp_nome == input$filtro_nome)
      }
      if (input$filtro_lote != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(lote == input$filtro_lote)
      }
    ...
    # 2. OBSERVADOR PARA ATUALIZAR AS OPÇÕES DE MODALIDADE
    observeEvent(c(input$filtro_distrito, input$filtro_concedente, input$filtro_nome, input$filtro_lote), {
      dados_intermediarios <- dados_brutos
      if (input$filtro_distrito != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(NM_DIST == input$filtro_distrito)
      }
      if (input$filtro_concedente != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(ppp_conced == input$filtro_concedente)
      }
    ...
    # 3. OBSERVADOR PARA ATUALIZAR AS OPÇÕES DE PODER CONCEDENTE
    observeEvent(c(input$filtro_distrito, input$filtro_modalidade, input$filtro_nome, input$filtro_lote), {
      dados_intermediarios <- dados_brutos
      if (input$filtro_distrito != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(NM_DIST == input$filtro_distrito)
      }
    ...
    # 4. OBSERVADOR PARA ATUALIZAR AS OPÇÕES DE NOME DA PARCERIA
    observeEvent(c(input$filtro_distrito, input$filtro_modalidade, input$filtro_concedente, input$filtro_lote), {
      dados_intermediarios <- dados_brutos
      if (input$filtro_distrito != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(NM_DIST == input$filtro_distrito)
      }
    ...
    # 5. NOVO OBSERVADOR PARA ATUALIZAR AS OPÇÕES DE LOTE
    observeEvent(c(input$filtro_distrito, input$filtro_modalidade, input$filtro_concedente, input$filtro_nome), {
      dados_intermediarios <- dados_brutos
      if (input$filtro_distrito != "Todos") {
        dados_intermediarios <- dados_intermediarios %>% filter(NM_DIST == input$filtro_distrito)
      }
